...
class StableDiffusionPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    ...
    def prepare_inpainting_latents(self, image, timestep, num_images_per_prompt, dtype, device, generator):
    ...
    def __call__(
        ...
        if layout_mask is None:
            raise ValueError("layout_mask couldn't be None!")
        ...
        # 4.5 Prepare inpaintings
        if is_editing:
            if not isinstance(is_editing[0], torch.FloatTensor):
                edit_before = preprocess_image(is_editing[0], 1)
            else:
                edit_before = is_editing[0]
            edit_msk = preprocess_mask(is_editing[1], 1, self.vae_scale_factor)
            timesteps, num_inference_steps = self.get_timesteps(num_inference_steps, is_editing[2], device)
            latent_timestep = timesteps[:1].repeat(1 * 1)

            latents, init_latents_orig, noise = self.prepare_inpainting_latents(
                edit_before, latent_timestep, 1, prompt_embeds.dtype, device, generator
            )

            mask = edit_msk.to(device=device, dtype=latents.dtype)
        ...
        # 6.2 Prepare mid stage image latents for loss guidance
        if do_latent_guidance:
            inp_img = np.array(mid_state_img).astype(np.float32) / 255.0
            inp_img = np.vstack([inp_img[None].transpose(0, 3, 1, 2)] * 1)
            inp_img = torch.from_numpy(inp_img)
            inp_img = 2.0 * inp_img - 1.0

            myimage = inp_img.to(device=device, dtype=latents.dtype)
            init_latent_dist = self.vae.encode(myimage).latent_dist
            init_latents = init_latent_dist.sample(generator=generator)
            init_latents = self.vae.config.scaling_factor * init_latents
            init_latents = torch.cat([init_latents] * 1, dim=0)

            noise = randn_tensor(init_latents.shape, generator=generator, device=torch.device(device), dtype=latents.dtype)
            init_latents = self.scheduler.add_noise(init_latents, noise, self.scheduler.timesteps)
            mid_state_latents = init_latents.unsqueeze(1)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if do_latent_guidance and i < 10:
                    latents = latents * (1 - mid_state_mask) + (latents * mid_state_mask * 0.2 + mid_state_latents[i] * mid_state_mask * 0.8)
                ...
                with torch.no_grad():
                    ...
                    if prompt_book_info[1] and i >= refine_step:
                        print('Stop patallel cross attention by step:',i)
                        cond, negative = prompt_embeds[0:1,:,:], prompt_embeds[-1:,:,:] #global, bg, cha1, cha2, ..., negative
                        prompt_embeds = torch.cat([cond,negative])
                        print(cond.size(), negative.size())
                        prompt_book_info[1] = False
